...
-- ============================================================================
-- 036_security_fix_price_tampering_prevention.sql
-- Purpose: Remove client price override vulnerability in booking creation
-- Critical Security Fix: Always calculate price server-side
-- ============================================================================

-- Drop old function that accepts client finalPrice
drop function if exists public.create_booking_atomic(
  uuid, bigint, bigint, uuid, text, uuid, date, time, text, text, numeric, numeric, text, text, text, numeric, numeric, jsonb
);

-- Recreate function WITHOUT p_final_price parameter
create or replace function public.create_booking_atomic(
  ...
  -- REMOVED: p_discount_amount (calculated server-side only)
  -- REMOVED: p_final_price (calculated server-side only)
...
-- Revoke from anonymous to prevent unauthorized booking creation
